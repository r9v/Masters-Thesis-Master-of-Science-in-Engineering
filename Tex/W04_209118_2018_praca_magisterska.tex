...
W naszym przypadku platformą manipulatora mobilnego jest monocykl. W założeniu ma poruszać się on bez poślizgu wzdłużnego i poprzecznego kół. Aby zapewnić spełnienie tych założeń należy nałożyć trzy ograniczenia. Można pokazać, że jedynie dwa z nich to ograniczenia nieholonomiczne, a ostatnie można zcałkować do ograniczenia holonomicznego i w rezultacie wyeliminować jedną współrzędną z $q_m$ \cite{7}. Aby jednak pokazać możliwość algorytmu w działaniu przy w obecności ograniczeń holonomicznych i niehololonomicznych potraktujmy wszystkie trzy jednakowo i nie całkujmy żadnego z nich.\\
\par Przedstawmy ograniczenia w postaci Pfaffa
\begin{equation}
\label{pfaff}
A(q)\dot{q}=0,
\end{equation}
Jako, że ograniczenia te dotyczą tylko platformy zapiszmy $A(q)$ następująco 
\begin{equation}
A(q)=\begin{bmatrix}
   A(q_m)&0\\
\end{bmatrix}
...
Niech baza jądra $A(q_m)$ będzie zbiorem wektorów $\{g_1(q_m), g_2(q_m),..., g_{n-l}(q_m)\}$. Zapiszmy prędkość $\dot{q}_m$ w następujący sposób 
\begin{equation}
...
Różnią się one wyborem funkcji wyjściowych $y(\cdot)$. Yamamoto i Yun założyli, że funkcje te będą zależne tylko od konfiguracji platformy $q_m$, natomiast Mazur pozwala aby funkcje wyjścia zależne był od konfiguracji platformy $q_m$, a także manipulatora $q_r$. Rozdział ten poświęcony jest opisowi obu podejść.
...
%\begin{itemize}
%\item pętla wewnętrzna - 
%\item pętla zewnętrzna - 
%\end{itemize}
\section{Podstawowy algorytm Yamamoto i Yun'a}
\subsection{Specyfikacja podejścia}
Yamamoto i Yun założyli, że manipulator ma ustawić się w stałej konfiguracji o maksymalnej manipulowalności, a zadanie jest realizowane jedynie przez ruch platformy. Podejście takie traktuje cały manipulator mobilny bardziej jako samą platformę do której przymocowany jest unieruchomiony manipulator, niż pełen system, gdzie platforma i manipulator działają równocześnie.\\
Podstawowy algorytm Yamamoto i Yun'a można stosować gdy manipulator ma pozostać nieruchomy np. .
\subsection{Funkcje wyjścia}
Przypomnijmy, że algorytm Yamamoto i Yun'a wymaga aby funkcje wyjścia były zależne jedynie od konfiguracji platformy tj. $y(q_m)$. Wybór tych funkcji należy podporządkować rodzajowi zadania od zrealizowania. Do śledzenia trajektorii dwuwymiarowej $(x_r,y_r)$
\subsection{Wybór konfiguracji manipulatora}
Ponieważ manipulator pozostaje przez czas działania algorytmu nieruchomy i nie uczestniczy czynnie w realizacji zadania należy ustawić go w takiej konfigurację, aby ułatwić platformie realizację zadania. Za przykładmem Yamamoto i Yun'a wybierzmy konfigurację maksymalizującą manipulowalność. Manipulowalność manipulatora zdefiniowana jest jako \cite{7}
\begin{equation}
\label{manipulowa}
w=\sqrt{\det(J(q_r)J^T(q_r))},
\end{equation} 
gdzie $J(q_r)$ to jakobian manipulatora. Jeśli manipulator jest redundantny manipulowalność (\ref{manipulowa}) można zredukować do
 \begin{equation}
w=|\det(J(q_r))|.
\end{equation} 
\subsection{Algorytm sterowania}
Odprzęganie i linearyzacja realizowane jest dzięki zastosowaniu do modelu (\ref{modelpp}) dwóch pętli sprzężenia zwrotnego.\\
 Pierwsza pętla - pętla wewnętrzna - przekształca układ do postaci liniowej typu "podwójny integrator".\\
Prawo sterowania pierwszej pętli to prawo z rodziny algorytmów 'obliczanych momentów'
\begin{equation}
\label{comTor}
u=B^{*-1}[C^*z+D^*+Q^*v].
\end{equation}
Po jej użyciu w modelu (\ref{modelpp}) uzyskamy liniowy układ, z nowym wejściem $v$ 
\begin{equation}
\label{eq:22}
\dot{z}=v.
\end{equation}
Druga pętla - zewnętrzna - zapewnia realizację zadania tj. śledzenia trajektori w uzyskanym układzie liniowym. Wprowadźmy prawo tej pętli w sposób następujący, najpierw zróżniczkujmy $y(q_m)$ dwukrotnie po czasie
\begin{equation}
\label{ydot}
\dot{y}(q_m)=\frac{dy(q_m)}{dq_m}\dot{q}_m=\frac{dy(q_m)}{dq_m}G_m(q_m)\eta=\Phi_m(q_m)\eta,
\end{equation}
\begin{equation}
\label{ypp}
...
\[
\Phi_m(q_m)= \frac{dy(q_m)}{dq_m}G_m(q_m).
\]\\
...
\end{equation}
do modelu (\ref{ypp}) otrzymamy 
\begin{equation}
\label{testtssts}
\ddot{y}=\zeta.
\end{equation} 
Wynikowy system (\ref{testtssts}) łatwo wysterować, na przykład za pomocą regulatora PD z korektą
\begin{equation}
\label{PD}
...
\par Ponieważ algorytm wykorzystuje odwrotność macierzy $\Phi$ należy  zapewnić, aby
\begin{equation}
\det(\Phi_m(q_m))\neq0
\end{equation}
...
\subsection{Specyfikacja podejścia}
Odmiennie od założeń Yamamoto i Yun'a rozszerzony algorytm nie wymaga bezruchu manipulatora. Dzięki temu platforma i manipulator mogą działać równolegle w sposób skoordynowany. Sterowanie platformy nie odbywa się w sposób bezpośredni, lecz działa w reakcji na dryf systemu.\\
Algorytm ten stosować można kiedy, np. sterowanie bazą jest niemożliwe(np. w przypadku robota free-floating).
\subsection{Funkcje wyjścia}
Funkcje wyjściowe mogą zależeć teraz od konfiguracji całego robota tj. i platformy i manipulatora. Przyjmują one postacie\cite{1}:
\begin{equation}
y(q)=\left(
\begin{array}{c}
y_1(q_m,q_r)\\
...
Liczba współrzędnych efektora w układzie ($X_0$,$Y_0$), które można odpsprząc zależy od liczby sterowań platformą. W przypadku platformy klasy $(2,0)$ możemy śledzić dwie współrzędne.\\
\subsection{Algorytm sterowania}
\par Identycznie jak w przypadku algorytmu podstawowego, algorytm z rozszerzonymi funkcjami wyjściowymi złożony jest z dwóch pętli sprzeżeń zwrotnych\cite{1}
\begin{itemize}[noitemsep,topsep=0pt]
...
u=B^{*-1}[C^*z+D^*+Q^*v].
\end{equation}
Zewnętrza pętla ma także postać identyczną jak w przypadku algorytmu podstawowego
\begin{equation}
\label{eq:petla2}
v=\Phi^{-1}[-\dot{\Phi} z+\zeta].
\end{equation}
Macierz $\Phi$ różni się jednak od poprzedniczki. Zależy ona od całej konfiguracji $q$, a nie tylko $q_m$ \[
\Phi(q)=
\begin{bmatrix}
    \Phi_m       & \Phi_{mr} \\
    0       & \Phi_r  \\
...
\end{bmatrix}.
\]
Prawo sterowania zewnętrznej pętli (\ref{eq:petla2}) wyprowadza się tak jak poprzednio.\\
...
\ddot{y}=\zeta.
\end{equation}
Otrzymany system jest identyczny jak ten otrzymany po zastosowaniu algorytmu Yamamoto i Yun'a i również można nim sterować używając regulatora PD z korektą(\ref{PD}).
\paragraph{Warunki działania algorytmu} Tak jak w przypadku podstawowego algorytmu Yamamoto i Yun'a należy zapewnić odwracalość macierzy $\Phi$. Warunek jaki trzeba spełnić aby algorytm funkcjonował jest więc następujący
\begin{equation}
