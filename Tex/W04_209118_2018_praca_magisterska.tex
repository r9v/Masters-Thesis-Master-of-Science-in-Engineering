...
Odprzęganie i linearyzacja realizowane jest dzięki zastosowaniu do modelu (\ref{modelpp}) dwóch pętli sprzężenia zwrotnego. Pierwsza pętla - pętla wewnętrzna - przekształca układ do postaci liniowej typu "podwójny integrator", zaś druga - zewnętrzna - zapewnia realizację zadania tj. śledzenia trajektori w uzyskanym układzie liniowym. \\
Prawo sterowania pierwszej pętli to prawo z rodziny algorytmów 'obliczanych momentów'
\begin{equation}
\label{comTor}
u=B^{*-1}[C^*z+D^*+Q^*v].
\end{equation}
Prawo drugiej pętli to
\begin{equation}
\label{eq:petla22}
v=\Phi^{-1}[-\dot{\Phi} z+\zeta].
\end{equation}
Macierz $\Phi$ jest równa
\[
\Phi(q_m)= \dot{y}(q_m)=\frac{dy}{dq_m}G_m.
\]
Sprawdźmy teraz poprawność algorytmu. Zastosujmy najpierw pierwszą pętlę (\ref{comTor}) do modelu (\ref{modelpp}). W wyniku otrzymamy 
\begin{equation}
\label{eq:22}
\dot{z}=v,
\end{equation}
gdzie $v$ jest nowym wejściem.
Teraz zróżniczkujmy $y(q_m)$ dwukrotnie po czasie
\begin{equation}
\label{ydot}
\dot{y}(q_m)=\frac{dy}{dq_m}\dot{q}_m=\frac{dy}{dq_m}G_mz=\Phi(q)z,
\end{equation}
\begin{equation}
\label{ypp}
\ddot{y}(q_m)=\dot{\Phi}z+\Phi \dot{z}=\dot{\Phi}z+\Phi v.
\end{equation} 
Łatwo sprawdzić, że po zastosowaniu prawa (\ref{eq:petla22}) do (\ref{ypp}) otrzymamy 
\begin{equation}
\label{testtssts}
\ddot{y}=\zeta.
\end{equation} 
