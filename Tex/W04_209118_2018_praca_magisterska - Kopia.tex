...
Różnią się one wyborem funkcji wyjściowych $y(\cdot)$. Yamamoto i Yun założyli, że funkcje te będą zależne tylko od konfiguracji platformy $q_m$, natomiast Mazur pozwala aby funkcje wyjścia zależne był również od konfiguracji samego manipulatora $q_r$. Rozdział ten poświęcony jest opisowi obu podejść.
...
Przypomnijmy, że algorytm podstawowy wymaga aby funkcje wyjścia były zależne jedynie od konfiguracji platformy tj. $y(q_m)$. Wybór tych funkcji należy podporządkować rodzajowi zadania od zrealizowania. Do śledzenia trajektorii dwuwymiarowej $(x_r,y_r)$
...
\dot{z}=v.
\end{equation}
Druga pętla - zewnętrzna - zapewnia realizację zadania tj. śledzenia trajektori w uzyskanym układzie liniowym. Prawo sterowania tej pętli jest następujące
\begin{equation}
\label{eq:petla22}
v=\left(
\begin{array}{c}
...
\]
Sprawdźmy poprawność algorytmu. Zróżniczkujmy $y(q_m)$ dwukrotnie po czasie
\begin{equation}
\label{ydot}
...
\begin{equation}
\label{ypp}
\ddot{y}(q_m,\dot{q}_m)=\dot{\Phi}_m(q_m,\dot{q}_m)\eta+\Phi_m(q_m) \dot{\eta}=\dot{\Phi}_m(q_m,\dot{q}_m)\eta+\Phi_m(q_m) v.
\end{equation} 
Po zastosowaniu prawa sterowania (\ref{eq:petla22}) do (\ref{ypp}) otrzymamy 
\begin{equation}
\label{testtssts}
\ddot{y}=\zeta.
\end{equation} 
